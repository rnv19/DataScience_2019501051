...
def dropFeturesWithNullValuesGreaterThanALimit(trainx_df, testx_df, null_ratio=0.3):
    ...
    columns_with_null_values = [[col, float(trainx_df[col].isnull().sum(
    ))/float(sample_size)] for col in trainx_df.columns if trainx_df[col].isnull().sum()]
    columns_to_drop = [
        x for (x, y) in columns_with_null_values if y > null_ratio]
...
def oneHotEncode(trainx_df, testx_df):
    categorical_columns = [
        col for col in trainx_df.columns if trainx_df[col].dtype == object]
    ordinal_columns = [
        col for col in trainx_df.columns if col not in categorical_columns]
    dummy_row = list()
    ...
    trainx_df = pd.concat(
        [trainx_df[ordinal_columns], trainx_enc], axis=1, ignore_index=True)
    testx_df = pd.concat(
        [testx_df[ordinal_columns], testx_enc], axis=1, ignore_index=True)
...
def fillMissingValues(trainx_df, testx_df):
    ...
    trainx_df_filled = pd.DataFrame(
        trainx_df_filled, columns=trainx_df.columns)
...
def splitTrainAndTest(trainx_df, trainy_df, split_ratio=0.3):
    X_train, X_test, y_train, y_test = train_test_split(
        trainx_df, trainy_df.values.ravel(), test_size=split_ratio, random_state=42)
...
def getRSqureandMSEVsAlphaPlots(X_train, X_test, y_train, y_test, alpha_start=0.1, alpha_end=10, jumps=10):
    ...
    for sigma in np.linspace(alpha_start, alpha_end, jumps):
        ...
        mse_train.append(
            round(mserr(y_train, Ridge_model.predict(X_train)), 4))
...
trainx_df, trainy_df, testx_df = readDataSets(
    "train.csv", "test.csv", predict_col='SalePrice', index_col="Id")
trainx_df, testx_df = dropFeturesWithNullValuesGreaterThanALimit(
    trainx_df, testx_df, null_ratio=0.5)
...
X_train, X_test, y_train, y_test = splitTrainAndTest(
    trainx_df, trainy_df, split_ratio=0.3)
LRModel = getLinearRegressionModel(X_train, y_train)
RidgeModel = getRidgeRegressionModel(
    X_train, y_train, tolerence=0.0001, reg_par=0.5)
getRSqureandMSEVsAlphaPlots(
    X_train, X_test, y_train, y_test, alpha_start=2, alpha_end=146, jumps=10)
